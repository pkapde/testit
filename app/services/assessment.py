...
def apply_claim_assessment(result: ClaimTriageResult) -> ClaimTriageResult:
    ...
    summary = (
        f"Claim {result.validation.claim_id} includes {len(evidence)} validated document(s) "
        f"and {extracted_count} extracted claim data point(s). "
        "Prepare a concise evidence briefing and practical validator checklist from the supplied claim package."
    )
